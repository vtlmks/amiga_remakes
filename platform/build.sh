...
# -DMKFW_TIMER_DEBUG "
	...
	"clean")
		rm -f *.gcda *.gcno *.gcov perf.data* *.alias *.o ${PROJECT_NAME} ${PROJECT_NAME}.exe viewer viewer.exe
		exit 0
		;;
	*)
...
# Build Linux version
(
	$CC $CFLAGS $LINUX_CFLAGS linux_platform.c -o ${PROJECT_NAME} $INCLUDE_PATHS $LINUX_INCLUDE $LINUX_LDFLAGS $LINUX_LIBS
) &

# Build Windows version
# (
# 	$WINCC $CFLAGS $WINDOWS_CFLAGS win32_platform.c -o ${PROJECT_NAME}.exe $INCLUDE_PATHS $WINDOWS_INCLUDE $WINDOWS_LDFLAGS $WINDOWS_LIBS
# ) &
