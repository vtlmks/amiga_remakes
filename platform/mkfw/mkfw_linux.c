		...
		switch(event.type) {
			...
			case MotionNotify: {
				if(PLATFORM(state)->in_window) {
					int delta_x = event.xmotion.x - PLATFORM(state)->last_mouse_x;
					int delta_y = event.xmotion.y - PLATFORM(state)->last_mouse_y;
					...
					if(PLATFORM(state)->mouse_constrained) {
						int center_x = PLATFORM(state)->win_saved_width / 2;
						int center_y = PLATFORM(state)->win_saved_height / 2;
...
static void mkfw_set_window_title(struct mkfw_state *state, char *title) {
