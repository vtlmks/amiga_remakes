...
- Safe to call multiple times (will cleanup and reinitialize)
...
```c
void mkui_text_colored(char *text, struct mkui_color color)
```
...
- Returns true only on mouse button release while hovering
...
```c
int mkui_checkbox(char *label, int *checked)
```
...
```c
int mkui_slider_float(char *label, float *value, float min_val, float max_val)
```
...
- Supports mousewheel scrolling (0.01 increment)
...
```c
int mkui_slider_int(char *label, int *value, int min_val, int max_val)
```
...
- Supports mousewheel scrolling (1 unit per scroll)
...
```c
int mkui_slider_int64(char *label, int64_t *value, int64_t min_val, int64_t max_val)
```
...
- Mousewheel increment scales with range (range / 100)
...
```c
int mkui_radio_button(char *label, int *selected, int value)
```
...
```c
int mkui_collapsing_header(char *label, int *open)
```
...
```c
int mkui_text_input(char *label, char *buffer, size_t buffer_size)
```
...
```c
int mkui_combo(char *label, int *current_item, char **items, int items_count)
```
...
```c
int mkui_listbox(char *label, int *current_item, char **items,
...
```c
void mkui_begin_window(char *title, float *x, float *y, float width, float height)
```
...
- Position can be modified by dragging title bar (future feature)
- Use pointers for x/y to enable draggable windows
...
```c
void mkui_image(uint32_t texture_id, float width, float height)
```
...
**Notes:**
- Texture must be already created and uploaded to OpenGL
- UV coordinates are (0,0) to (1,1) covering full texture
- Useful for icons, sprites, or preview images
...
```c
void mkui_image_rgba(uint8_t *rgba_buffer, int width, int height)
```

Display raw RGBA pixel data.

**Parameters:**
- `rgba_buffer` - RGBA pixel data (4 bytes per pixel)
- `width` - Image width in pixels
- `height` - Image height in pixels

**Notes:**
- Creates temporary texture from pixel data
- Convenient for dynamic images or screenshots
- Less efficient than pre-uploaded textures
...
style->colors[MKUI_COLOR_TEXT] = mkui_rgb(1.0f, 1.0f, 0.0f);  // Yellow text
style->item_spacing = 8.0f;  // More spacing between widgets
```
...
**Parameters:**
- `color_id` - Color identifier (see `enum mkui_color_id`)
- `r` - Red component (0.0 to 1.0)
- `g` - Green component (0.0 to 1.0)
- `b` - Blue component (0.0 to 1.0)
...
**Color IDs:**
- `MKUI_COLOR_TEXT`
- `MKUI_COLOR_WINDOW_BG`
- `MKUI_COLOR_BORDER`
- `MKUI_COLOR_BUTTON`
- `MKUI_COLOR_BUTTON_HOVERED`
- `MKUI_COLOR_BUTTON_ACTIVE`
- `MKUI_COLOR_FRAME_BG`
- `MKUI_COLOR_FRAME_BG_HOVERED`

**Example:**
```c
// Set button color to green
mkui_style_set_color(MKUI_COLOR_BUTTON, 0.0f, 0.8f, 0.0f, 1.0f);

// Make window background more transparent
mkui_style_set_color(MKUI_COLOR_WINDOW_BG, 0.06f, 0.06f, 0.06f, 0.7f);
```
...
See [mkfw_ui_example.c](../mkfw_ui_example.c) for a complete working example demonstrating all widgets.
...
The MKFW UI subsystem maintains the same MIT license as MKFW core.

See [mkfw.h](../mkfw.h) for full license text.
