...
**Usage:**
```c
mkui_begin_window("My Window", 50, 50, 400, 300);
...
```c
mkui_begin_window("Settings", 100, 100, 350, 400);
...
1. **Layout spacing** is controlled by `g_mkui_ctx->item_spacing` (default: 4px)
2. **Padding** inside widgets is `g_mkui_ctx->frame_padding` (default: 4px)
3. **Window padding** is `g_mkui_ctx->window_padding` (default: 8px)
