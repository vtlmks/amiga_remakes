...
MKFW is a minimal, single-header windowing and input library for OpenGL 3.1+ applications. It provides a simpler alternative to GLFW with platform-specific implementations for X11/Linux and Windows.

## Features

- Multiple independent window support
- Window creation and management
- OpenGL context creation (Core Profile 3.1+)
...
**Notes:**
- Creates an OpenGL 3.1 Core Profile context
- Sets up X11/Win32 event handling
- Enables XInput2 raw motion on Linux
- Installs signal handlers (SIGINT, SIGTERM, etc.)
- Returns NULL on failure instead of calling exit()
- You can create multiple windows by calling this multiple times
...
// Both windows operate completely independently
...
```c
void mkfw_set_window_title(struct mkfw_state *state, char *title)
```
...
**Returns:**
- Non-zero if user requested close (ESC key, window close button, or signal)
...
```c
uint8_t *keyboard_state = mkfw_get_keyboard_state(state);
```

Each window state contains a keyboard state array tracking current key states.
...
```c
void mkfw_update_keyboard_state(struct mkfw_state *state)
```

Update previous keyboard state. Call once per frame after processing input.
...
```c
void mkfw_update_mouse_state(struct mkfw_state *state)
```

Update previous mouse button state. Call once per frame.
...
### `mkfw_show_mouse_cursor`

```c
void mkfw_show_mouse_cursor(struct mkfw_state *state, int visible)
```
...
**Notes:**
- Uses XInput2 raw motion on Linux
...
```c
void mkfw_sleep(struct mkfw_state *state, uint64_t nanoseconds)
```
...
int main(void) {
    ...
    while (!mkfw_should_close(main_window) && !mkfw_should_close(options_window)) {
        ...
        // Update state tracking
        mkfw_update_keyboard_state(main_window);
        mkfw_update_mouse_state(main_window);
        mkfw_update_keyboard_state(options_window);
        mkfw_update_mouse_state(options_window);
    }
...
- Creates OpenGL 3.1 Core Profile context via `glXCreateContextAttribsARB`
- Handles `_NET_WM_STATE` for fullscreen
- Installs signal handlers for cleanup (SIGINT, SIGTERM, SIGHUP, etc.)
...
- Implementation in `mkfw_windows.c`
...
- `mkfw_x11.c` - X11/GLX implementation for Linux
- `mkfw_windows.c` - Win32/WGL implementation for Windows
...
- **OpenGL Version:** 3.1 Core Profile
- **GLX Version:** 1.3+ (for `glXCreateContextAttribsARB`)
- **XInput Version:** 2.0+ (for raw motion)

The library provides a minimal API surface focused on modern OpenGL applications. Legacy features (OpenGL 2.x, deprecated functions) are not supported.
...
- Fixed OpenGL 3.1 Core Profile (no version selection)
...
- **Built-in signal handling** - Automatic cleanup on SIGINT/SIGTERM
...
- `mkfw_x11.c` - X11/Linux implementation details
- `mkfw_windows.c` - Windows implementation details
